...
ticker = input("Enter ticker symbol here: ") # this is the short name of currency or company symbols
from_date = input('Enter start date in yyyy/mm/dd format: ')
to_date = input('Enter end date in yyyy/mm/dd format: ')

from_datetime = datetime.strptime(from_date, '%Y/%m/%d') # convert to str
to_datetime = datetime.strptime(to_date, '%Y/%m/%d')

from_epoch = int(time.mktime(from_datetime.timetuple())) # convert to epoch
